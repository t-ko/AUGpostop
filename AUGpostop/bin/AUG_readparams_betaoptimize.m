function cmd = AUG_readparams_betaoptimize(filename,func,eval_functions)
    if (nargin < 2)
    ...
    delimiter = '##';
    params = readtable([ '..' filesep 'studydata' filesep filename ],'ReadVariableNames',0,'ReadRowNames',1);
    regionlabels = {'Left Forehead','Right Forehead','Left Parietal','Right Parietal'};
    cmd = '';
    for d = 1:size(params,2)
        for i = 1:size(params,1)
            params{i,d} = strtrim(params{i,d});
        end
        p = params{:,d};
        if iscell(p(1))
            if isempty(func) || ~isempty(strfind(func_str,'OptProps'))
                ...
                if ~isempty(strmatch(p{3},'absolutes'))
                    fn = sprintf('OptProps_%s_%s_1.m',p{2},lower(p{1}));
                else
                    fn = sprintf('OptProps_%s_%s.m',p{2},lower(p{1}));
                end                
                cmd = [cmd sprintf('run(''%s'');',fn)];
                w1 = fopen(fn, 'w');
                fwrite(w1,output);
                fclose(w1);
            ...
            if isempty(func) || ~isempty(strfind(func_str,'absprops'))
                ...
                % Write output to formatted filename
                if ~isempty(strmatch(p{3},'absolutes'))
                    fn = sprintf('absprops_%s_%s_1.m',p{2},lower(p{1}));                    
                else
                    fn = sprintf('absprops_%s_%s.m',p{2},lower(p{1}));
                end
                cmd = [cmd sprintf('run(''%s'');',fn)];
                w1 = fopen(fn, 'w');
                fwrite(w1,output);
    ...
    if isempty(func) || ~isempty(strfind(func_str,'PlotAbsProps'))
        ...
        ID = ID{1};
        patdate = params{2,1};
        fn = sprintf('PlotAbsProps_%s_%s.m',patdate{1},ID(1:6));
        cmd = [cmd sprintf('run(''%s'');',fn)];
        w1 = fopen(fn, 'w');
        fwrite(w1,output);
        fclose(w1);
    ...
    for d = 1:size(params,2)
        ...
        if iscell(p(1))
            if isempty(func) || (~isempty(strfind(func_str,'DbFitAbs')) && isempty(strfind(func_str,'PlotDbFitAbs'))) || (length(strfind(func_str,'DbFitAbs'))>1)
                %% DbFitAbs
                filesplit = importtemplate('DbFitAbs_YYYYMMDD_augNNN_MMDDYY_betaoptimize.m',delimiter);
                p_format(1) = p(1);
                p_format(2) = p(10);
                p_format(3) = p(11);
                p_format(4) = p(3);
                ...
                p_format(7) = {[rSTR{:}]};
                % Populate string with probe source-detector separations
                try
                    if strcmp(p{26},'trapezoid')
                        p_format(8) = {'2.5 2.5 2.5 2.5 2.5 2.5 2.5 0.8'};
                    elseif strcmp(p{26},'custom1')
                        p_format(8) = {'2.75 2.75 2.75 2.75 2.75 2.75 2.75 2.75'};
                    else
                        p_format(8) = {'2.5 2.5 2.5 2.5 2.5 2.5 2.5 2.5'};
                    end
                catch
                    p_format(8) = {'2.5 2.5 2.5 2.5 2.5 2.5 2.5 2.5'};
                end
                ...
                if ~isempty(strmatch(p{10},'BFI_1'))
                    fn = sprintf('DbFitAbs_%s_%s_1.m',p{2},lower(p{1}));
                else
                    fn = sprintf('DbFitAbs_%s_%s.m',p{2},lower(p{1}));
                end
                cmd = [cmd sprintf('run(''%s'');',fn)];
                w1 = fopen(fn, 'w');
                fwrite(w1,output);
        ...
        %         run(sprintf('DbFitAbs_%s_%s.m',p{2},lower(p{1})));
            ...
            if isempty(func) || ~isempty(strfind(func_str,'Checkfits'))
                ...
                % Write output to formatted filename
                if ~isempty(strmatch(p{10},'BFI_1'))
                    fn = sprintf('Checkfits_%s_%s_1.m',p{2},lower(p{1}));
                else
                    fn = sprintf('Checkfits_%s_%s.m',p{2},lower(p{1}));
                end
                % cmd = [cmd sprintf('run(''%s'');',fn)]; % MANUAL RUN
                w1 = fopen(fn, 'w');
                fwrite(w1,output);
                fclose(w1);
                clear p_format output filesplit 
    ...
    if isempty(func) || ~isempty(strfind(func_str,'PlotDbFitAbs'))
        ...
        ID = ID{1};
        patdate = params{2,1};
        fn = sprintf('PlotDbFitAbs_%s_%s.m',patdate{1},ID(1:6));
%         cmd = [cmd sprintf('run(''%s'');',fn)]; % MANUAL RUN
        w1 = fopen(fn, 'w');
        fwrite(w1,output);
        fclose(w1);
