function cmd = AUG_readparams_betaoptimized(filename,func,eval_functions)
    if (nargin < 2)
    ...
    for d = 1:size(params,2)
        ...
        if iscell(p(1))
            if isempty(func) || (~isempty(strfind(func_str,'DbFitAbs')) && isempty(strfind(func_str,'PlotDbFitAbs'))) || (length(strfind(func_str,'DbFitAbs'))>1)
                %% DbFitAbs
                filesplit = importtemplate('DbFitAbs_YYYYMMDD_augNNN_MMDDYY_betaoptimized.m',delimiter);
                p_format(1) = p(1);
                p_format(2) = p(10);
                p_format(3) = p(11);
                p_format(4) = p(3);
