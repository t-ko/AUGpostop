...
%% Import patient absolutes: average mua/musp (TK 2015-05-07)
% syntax: PT001_060815_righthand_2p5cm_righthand_dpfout_withabs.mat
if exist([  studyID '_' analysisID ext '_dpfout_withabs.mat'],'file') %oxy data with absolutes
    tt=[ 'load ' studyID '_' analysisID ext '_dpfout_withabs.mat' ];
    eval(tt);
    marksforgamma = baselinemarks;
    if threelambda
...
elseif exist([ studyID '_' analysisID ext '_dpfout.mat'],'file') %relative oxy data
    tt=[ 'load ' studyID '_' analysisID ext '_dpfout.mat' ];
    eval(tt);
...
for measnum=0:size(intensitydata,1)-numframestoavg %UPDATED (TK) 02-22-15 to only include marked frames
    if (measnum+numframestoavg <= size(intensitydata,1)) && ~isempty(find(ONframes==measnum))
        if fitavg==1
...
if ~fitavg
    ...
    set(h1,'LineWidth',3, 'Marker','.', 'MarkerSize', 20)
    set(h2,'LineWidth',3)
    xlabel('Frame','FontSize',25)
...
%             end
%             analysis_date(R) = datenum([DateVector(R,1:3) 0 0 0]);
%         end      
%         
%         for rn = 1:length(regionmarks)
%             rmarks = Marksflow(regionmarks{rn});
%             if ~isempty(findstr(analysisID,'eft')) && ~isempty(findstr(analysisID,'orehead')) && isempty(findstr(analysisID,'0p75cm'))
...
%             elseif ~isempty(findstr(analysisID,'eft')) && ~isempty(findstr(analysisID,'eg')) && isempty(findstr(analysisID,'0p75cm'))
%                 mua(rmarks(1):rmarks(2)) = DateWeightedProp(analysis_date, mualeftleg);
%                 musp(rmarks(1):rmarks(2)) = DateWeightedProp(analysis_date, muspleftleg);
%                 sprintf('Left Leg - mua: %s, musp: %s', num2str(mua(rmarks(1))), num2str(musp(rmarks(1))))
%             elseif ~isempty(findstr(analysisID,'ight')) && ~isempty(findstr(analysisID,'orehead')) && isempty(findstr(analysisID,'0p75cm'))
...
%             elseif ~isempty(findstr(analysisID,'ight')) && ~isempty(findstr(analysisID,'eg')) && isempty(findstr(analysisID,'0p75cm'))
%                 mua(rmarks(1):rmarks(2)) = DateWeightedProp(analysis_date, muarightleg);
%                 musp(rmarks(1):rmarks(2)) = DateWeightedProp(analysis_date, musprightleg);
%                 sprintf('Right Leg - mua: %s, musp: %s', num2str(mua(rmarks(1))), num2str(musp(rmarks(1))))
%             end
%         end 
...
%                 elseif ~isempty(findstr(analysisID,'eft')) && ~isempty(findstr(analysisID,'eg')) && isempty(findstr(analysisID,'0p75cm'))
%                     mua(rmarks(1):rmarks(2)) = nanmean(leftlegmua(:,3));
%                     musp(rmarks(1):rmarks(2)) = nanmean(leftlegmusp(:,3));
%                 elseif ~isempty(findstr(analysisID,'ight')) && ~isempty(findstr(analysisID,'orehead')) && isempty(findstr(analysisID,'0p75cm'))
%                     mua(rmarks(1):rmarks(2)) = nanmean(rightmua(:,3));
%                     musp(rmarks(1):rmarks(2)) = nanmean(rightmusp(:,3));
%                 elseif ~isempty(findstr(analysisID,'ight')) && ~isempty(findstr(analysisID,'eg')) && isempty(findstr(analysisID,'0p75cm'))
%                     mua(rmarks(1):rmarks(2)) = nanmean(rightlegmua(:,3));
%                     musp(rmarks(1):rmarks(2)) = nanmean(rightlegmusp(:,3));
%                 end
%             catch
%             end
%         end        
