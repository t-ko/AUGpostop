...
regionmarks = {[1 2];[3 4];[6 7]}; % leave empty cell {} if no region discrimination
regionlabels = {'Left Forehead';'Right Forehead';'Right Parietal'};
marksforgamma = [6 7];
...
cutoff=1.3;%where to cut correlation curve off when fitting
...
% end
    ...
    if exist([ studyID '_' absID '_baselines.mat'],'file')
        ...
        eval(tt);
        for r = 1:length(regionmarks)
            rmarks = Marksflow(regionmarks{1});
            if ~isempty(findstr(regionlabels{r},'eft')) && ~isempty(findstr(regionlabels{r},'orehead'))
                mua(rmarks(1):rmarks(2)) = mean(leftmua(:,3));
                musp(rmarks(1):rmarks(2)) = mean(leftmusp(:,3));
            elseif ~isempty(findstr(regionlabels{r},'eft')) && (~isempty(findstr(regionlabels{r},'ietal')) || ~isempty(findstr(regionlabels{r},'ide')))
                mua(rmarks(1):rmarks(2)) = mean(leftparietalmua(:,3));
                musp(rmarks(1):rmarks(2)) = mean(leftparietalmusp(:,3));
            elseif ~isempty(findstr(regionlabels{r},'ight')) && ~isempty(findstr(regionlabels{r},'orehead'))
                mua(rmarks(1):rmarks(2)) = mean(rightmua(:,3));
                musp(rmarks(1):rmarks(2)) = mean(rightmusp(:,3));
            elseif ~isempty(findstr(regionlabels{r},'ight')) && (~isempty(findstr(regionlabels{r},'ietal')) || ~isempty(findstr(regionlabels{r},'ide')))
