...
%% FILENAME PARAMETERS
studyID='AUG093_020615'; %UPDATE to match data directory
prefix='BFI_020815'; %UPDATE to match BFI data prefix
analysisID='BFI_020815abs_fitavg'; %<abs> if fit with FD-DOS, <fitavg> if averaging detectors
absID='absolutes_020815'; %UPDATE to match FD-DOS data directory
%Extension added to filename, if nothing, just use ext='';
exten={''};

%% DCS PARAMETERS
s=1; %source select, 1 or 2
usedflowdets=[4 5 6 7 8];%must be consecutive, cannot be ex. 1,3,4 or 2,4.  must change code if this is the case 
fitavg = 1; %average all usedflowdets during correlation fitting?
regionmarks = {[1 2]}; % leave empty cell {} if no region discrimination
regionlabels = {'Left Forehead'};
marksforgamma = [1 2];
