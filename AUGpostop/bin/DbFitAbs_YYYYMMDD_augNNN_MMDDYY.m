...
usedflowdets=[##];%must be consecutive, cannot be ex. 1,3,4 or 2,4.  must change code if this is the case 
fitavg = 1; %average all usedflowdets during correlation fitting?
regionmarks = {##}; % leave empty cell {} if no region discrimination
regionlabels = {##};
marksforgamma = [1 2];
...
% end
    ...
    for measnum=ONframes %UPDATED (TK) 02-22-15 to only include marked frames
        if measnum+numframestoavg <= size(intensitydata,1)
            if fitavg==1
