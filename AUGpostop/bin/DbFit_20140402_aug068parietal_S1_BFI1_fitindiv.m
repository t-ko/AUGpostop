...
fitavg = 0; %average all usedflowdets during correlation fitting?
marksforgamma = [1 2];
syncmark=1;
...
% end
    ...
    %% Fit each source data separately
    ...
    for measnum=Marksflow(1):Marksflow(end)
        if fitavg==1
...
if ~fitavg
    ...
    muao=mua;
    ff=['save ' studyID '_S' num2str(s) '_' analysisID '_flow_output_fitindiv.mat timeaxis_flow mua muao muspo taus Dbfit Curvefit Curvefitg2 errorfit meanerror stderror corrs g1 intensitydata Marksflow Betasave numframestoavg fval exitflag usedflowdets fitbeta fitavg startcorr datalength avgnum cutoff'];
    eval(ff);
else
    %Make sure fit converged
    ...
    muao=mua;
    ff=['save ' studyID '_S' num2str(s) '_' analysisID '_flow_output_fitavg.mat timeaxis_flow muao muspo taus Dbfitavg Curvefitavg Curvefitg2avg corrsavg g1avg intensityavg Marksflow numframestoavg fvalavg exitflagavg Betasaveavg usedflowdets fitbeta fitavg startcorr datalength avgnum cutoff'];
    eval(ff);
