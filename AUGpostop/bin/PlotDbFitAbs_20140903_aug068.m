...
prefixes={'BFI_1';'BFI_2'};
analysisIDs={'BFI1abs_fitavg';'BFI2abs_fitavg'}; %R1, R2, R3
avgfit=[0 0]; %0 if averaged, channel number if not
plotID='BFIabs_fitavg';
names = prefixes;
sources=[0 0]; % source selection, 1 or 2, 0 if sole source;

baselinemarks=[1 2]; % Must refer to last studyID/analysisID

autoloadmarks=0; % Autoload marks below(TK 11-11-2014)
if ~autoloadmarks
    markstoshow=[1 2 3 4];
...
for R = 1:length(analysisIDs)
    ...
    if s
        region(R).t = (t0+timeaxis_flow(s:2:end)-timeaxis_flow(1)).*60;
    else
        region(R).t = (t0+timeaxis_flow-timeaxis_flow(1)).*60;
    end
    region(R).t = region(R).t(Marksflow(min(markstoshow)):Marksflow(max(markstoshow)));
