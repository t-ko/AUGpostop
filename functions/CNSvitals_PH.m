function [eventtimes,events]=CNSvitals_4PH(subjectid,patdate,ext,savefigures,timeaxis,Markstime,timesyncmark_dcs,savematfile,rotatelabel,invivo,hrmin,bpmin,bpmax,hrmax,spo2min,rapmin,mapnans)
...
if invivo
    ...
    fnametmp=[ fdir 'EtCO2,na,Numeric,Float,Solar,data_part1of1.mat' ];
end
if exist(fnametmp)
    load([ fnametmp ]);
    etco2=measurement_data;
...
if exist(fnametmp)
    ...
    hold on, plot(50,50,'.k','MarkerSize',80)
    set(gca,'FontSize',34)
    xlim([min(timeaxis) max(timeaxis)])
    ylim(SpO2lim)
    tmplim3=get(gca,'YLim');
