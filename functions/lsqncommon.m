...
%   $Revision: 1.1.6.6 $  $Date: 2011/05/09 01:16:17 $
...
largeScaleFlag = true;  % default
if isfield(options,'LargeScale') && ~isempty(options.LargeScale)
    % 1 means large-scale, 0 means medium-scale
...
if trustRegion && ~(largeScaleFlag || levMarq)
    error(message('optimlib:lsqncommon:GNremoval'));
elseif trustRegion && ~largeScaleFlag && levMarq
    error(message('optimlib:lsqncommon:AlgorithmConflict'));
end
