...
%   $Revision: 1.1.6.10 $  $Date: 2011/05/09 01:16:29 $
...
% Initialize fault tolerance structure. For more information on the
% fault tolerance structure, see the help for faultTolStructManager.
faultTolStruct = faultTolStructManager('create', false, false);
...
while ~ex 
    ...
    if verb > 1
        if iter > 0
            if faultTolStruct.undefObj 
                fprintf('User objective function returned %s; trying a new point...\n', ...
   ...
   faultTolStruct = faultTolStructManager('reset', faultTolStruct);
   ...
   if ~ex 
      ...
      %       Perturb?
      [pert,newx] = perturbTrustRegionReflective(newx,l,u);
      ...
      faultTolStruct = faultTolStructManager('update', faultTolStruct, ...
          newfvec, verb > 1);
